...
def _sanitised_returns(series: pd.Series) -> pd.Series:
    """``pct_change`` returns with non-finite values dropped.

    A zero amount makes ``pct_change`` divide by zero and yield ``inf``;
    a full series of such returns poisons downstream ``std()`` calls. Drop
    them so volatility/hist-vol are always finite.
    """
