"""Reconciliation sheet builder - cross-source reconciliation of SAP ledger signals and EDF invoices.

Extracted from ``edf_collector.py`` as part of the modularization refactor
(Task 5 - Phase 4).  ``write_reconciliation_sheet`` writes a single
``Reconciliation`` sheet to the evidence workbook.

Compat re-export lives in ``edf_collector.py`` so callers using
``from edf_collector import write_reconciliation_sheet`` continue to work;
stripped by Task 7.

Note: ``detect_reconciliation_statement`` and ``extract_reconciliation_statement_rows``
were originally placed in this module's test contract but the canonical home
(per the test files) is ``processors.detection``.  Importable from both
locations during the migration.
"""
...
def _recon_parse_iso_date(s: str) -> str:
    ...
    try:
        ...
        return "N/A"  # type: ignore[no-any-return]


def _recon_amount_to_float(s: str) -> float:
...
def _recon_hyperlink(ws: Worksheet, row: int, col: int, sheet: str, target_row: int) -> None:
    """Place an internal-workbook hyperlink at (row, col) targeting ``sheet!Atarget_row``."""
    from openpyxl.styles import Font as _Font

    cell = ws.cell(row=row, column=col)
    cell.value = f'=HYPERLINK("#{sheet}!A{target_row}", "jump")'
...
def write_reconciliation_sheet(
    ...
    def _build_section(
        ...
        for sap in sap_items:
            ...
            for i in unmatched_inferred[:]:
                inf = inferred_rows[i]
                _inf_date_str = _recon_parse_iso_date(inf.get(inferred_date_key, ""))
                inf_date = date.fromisoformat(_inf_date_str) if _inf_date_str and _inf_date_str != "N/A" else pd.NaT
                date_close = (
    ...
    for sap in sap_financial:
        _sap_date_str = _recon_parse_iso_date(sap.get("Posting Date", ""))
        sap_date = date.fromisoformat(_sap_date_str) if _sap_date_str and _sap_date_str != "N/A" else pd.NaT
        sap_amt = _recon_amount_to_float(sap.get("Amount", ""))
        matched = False
        for i in unmatched_ev[:]:
            ev = evidence_rows_list[i]
            _ev_date_str = _recon_parse_iso_date(ev.get("Date", ""))
            ev_date = date.fromisoformat(_ev_date_str) if _ev_date_str and _ev_date_str != "N/A" else pd.NaT
...
__all__ = [
    ...
    # Re-exports from detection for backward-compat with the test contract
    "detect_reconciliation_statement",
    "extract_reconciliation_statement_rows",
]
